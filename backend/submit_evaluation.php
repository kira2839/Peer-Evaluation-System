...
function insertIntoTable($form_data)
{
    ...
    for ($i = 0; $i < count($form_data); $i++) {
        ...
        if ($studentEvaluationModel->insert($student_id, $group_member_id, $role, $leadership, $participation,
                $professionalism, $quality, $normalized_score) === true OR
            $studentEvaluationModel->update($student_id, $group_member_id, $role, $leadership, $participation,
                $professionalism, $quality, $normalized_score) === true) {
